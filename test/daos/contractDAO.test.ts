...
test('save a contract', async () => {
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract1);
...
test('get contracts if expecting nothing', async () => {
    let updatedCorpContract: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
...
test('get contracts if expecting 2', async () => {
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract1);
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract2);
    let updatedCorpContract: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
...
test('update an existing contract', async () => {
    ...
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract_1_1);
    let count: number = await provider.connection.collection(`${corporationId}_contracts`).find({}).count();
    expect(count).toBe(1);
    let isSaved: boolean = await CorpContractQueries.isPresent(provider, corporationId, corpContract1);
    expect(isSaved).toBe(true);
    let updatedCorpContract: Contract = (await CorpContractQueries.getContracts(provider, corporationId))[0];
    expect(updatedCorpContract.status).toBe(IStatus.finished);
});

test('delete contract does nothing if provided with nothing', async () => {
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract1);
    await CorpContractQueries.deleteContract(provider, corporationId, null);
    let updatedCorpContract: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
    expect(updatedCorpContract.length).toBe(1);
});
...
test('delete multiple contracts does nothing if provided with nothing', async () => {
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract1);
    await CorpContractQueries.deleteContracts(provider, corporationId, null);
    let updatedCorpContract: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
    expect(updatedCorpContract.length).toBe(1);
});
...
test('removeOldContracts successfully removes corpContract1 as it was old', async () => {
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract1);
    await CorpContractQueries.saveOrUpdateContract(provider, corporationId, corpContract2);
    let oldCorpContracts: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
    ...
    let remainingCorpContracts: Contract[] = await CorpContractQueries.getContracts(provider, corporationId);
